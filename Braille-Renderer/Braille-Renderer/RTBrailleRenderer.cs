...
{
   public class RTBrailleRenderer
    { //todo: rest einfügen, Pfad zu Liblouis, Pfad zu Tabellen, BReite
        ...
        public RTBrailleRenderer() : this(pathToLiblouis, pathToTables)
        {  }
        ...
        {
            //for (int i = 0; i < dom.Length; i++){    ======TEsting first element
            for (int i = 0; i < dom.Length; i++){
                ...
                if (!(htmlelement == null)){
                    coords.CurrentX = 0;
                    NewElement(ref M, htmlelement, rules);
                }
            }
        ...
        //public void ApplyCssToHtmlElement(ref List<bool[]> M, IDomObject htmlelement, Dictionary<String, List<String>> rules)
        //{
        ...
        //    // match inner Tags, indezieren, rekursiver Aufruf
        //    NewElement(ref M, nodes, selector, rules);
        ...
        /// <param name="rules">The css rules.</param>
        void NewElement(ref List<bool[]> M, IDomObject htmlelement, Dictionary<String, List<String>> rules)
        {
        ...
        void NewElement(ref List<bool[]> M, IDomObject htmlelement, Dictionary<String, List<String>> rules, ref uint x, ref uint y, ref uint x_max, ref uint x_min)
        {
            string selector = RTBrailleRendererHelper.getSelectorFromElement(htmlelement);
