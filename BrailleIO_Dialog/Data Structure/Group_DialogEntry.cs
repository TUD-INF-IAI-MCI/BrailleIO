...
{
    /// <summary>
    /// Special DialogEntry for grouping other <see cref="DialogEntry"/> semantically or functional.
    /// This class contains special handling grouping entries.
    ...
    public class Group_DialogEntry : SelfRenderingDialogEntry, IIteratable
    {
        ...
        {
            if (children != null) ChildEntries.AddRange(children);
        }
        ...
        /// <summary>
        /// Resets the group of this entry. Clears the list of children and resets the type.
        /// </summary>
        public void ResetGroupList()
        {
            var children = GetChildEntryList();
            if (children != null && children.Count > 0)
                foreach (var item in children)
                {
                    RemoveFromGroupList(item as DialogEntry);
                }
        }
        ...
        /// <summary>
        /// Gets the selected child entry if one is elected.
        /// </summary>
        /// <returns>The selected group entry or <c>null</c></returns>
        public DialogEntry GetSelectedGroupEntry()
        {
            return GetFirstChildElementByStatus(DialogEntryStatus.Selected);
        }

        /// <summary>
        /// Gets the active child entry if one is activated.
        /// </summary>
        /// <returns>The activated group entry or <c>null</c></returns>
        public DialogEntry GetActiveGroupEntry()
        {
            return GetFirstChildElementByStatus(DialogEntryStatus.Activated);
        }
